...
{
    NSString * cKey = [key substringWithRange:NSMakeRange(0, 32)];
    NSString * cIV = [key substringWithRange:NSMakeRange(32, 16)];
    return  [self encrypt:plainText key:cKey iv:cIV];
}
...
- (NSData *)decrypt:(NSData *)encryptedText key:(NSString *)key
{
    NSString * cKey = [key substringWithRange:NSMakeRange(0, 32)];
    NSString * cIV = [key substringWithRange:NSMakeRange(32, 16)];
    return  [self decrypt:encryptedText key:cKey iv:cIV];
}
